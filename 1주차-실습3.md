...
## 전체코드
[1주차-실습3.cpp](1주차-실습/1주차-실습3.cpp)
## 부분 설명
파노라마를 만드는 코드에 대해서는 [실습2](1주차-실습/1주차-실습2.cpp)에서 다뤘으므로 그것을 보시길 바랍니다.<br>
Main 함수부터 자세히 보면
```
...
다른점  findHomography함수에서 마지막에 인자로 mask를 넣어 주면 
```
    Mat panorama_stiching(Image_feature img1, Image_feature img2)
```
사진 두개의 feature를 받아서 두개를 stiching 한 영상을 결과롤 return 해줍니다.<br>
이 함수가 실습 2 함수에서 한부분과 다른 점은 기준 영상에 오른쪽으로만 영상을 붙혔다면<br>
이건 어느 방향으로든 영상을 붙힐 수 있습니다.<br>
```
        Mat matResult;
	Mat matPanorama;
...
로 P_Trans_conerPt에 img2를 변환한 img를 넣습니댜.
```
        double min_x, min_y, max_x, max_y, bef_max_x;
	float min_x1, min_x2, min_y1, min_y2, max_x1, max_x2, max_y1, max_y2;
